...
export class PersonalityTestComponent implements OnInit {
  ...
  onSubmit() {
    if (this.personalityTestForm.invalid) {
      console.log(this.personalityTestForm.controls);
      return;
    }
    ...
    this.usersService
      .updateUser(u._id, {
        personalityResult: this.getResult(),
      })
      .subscribe({
        next: () => {
          swal.fire(
          ...
          this.emailService
            .sendResult({
              subject: 'Resultados del Test de Personalidad',
              test: 'Personalidad',
              to: email,
